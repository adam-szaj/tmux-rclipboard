...
tmux bind-key -T copy-mode-vi y send -X copy-pipe-and-cancel "RCLIP_BIN=${RCLIP_BIN} RCLIP_TOPIC=${RCLIP_TOPIC} RCLIP_APP=${RCLIP_APP} ${SCRIPT_DIR}/scripts/publish.sh"
tmux bind-key ] run-shell "RCLIP_BIN=${RCLIP_BIN} RCLIP_TOPIC=${RCLIP_TOPIC} RCLIP_APP=${RCLIP_APP} ${SCRIPT_DIR}/scripts/paste.sh"
...
if [ "${RCLIP_STATUS}" = "on" ]; then
    ...
    if [[ "$current" != *"health.sh"* ]]; then
        seg="#[fg=colour244]rc:#[default]#(RCLIP_BIN=${RCLIP_BIN} ${SCRIPT_DIR}/scripts/health.sh)"
